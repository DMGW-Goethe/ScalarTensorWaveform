...
# This function returns the upper incomplete gamma function as defined here https://mathworld.wolfram.com/IncompleteGammaFunction.html
def upper_gammainc(a, x):
...
class ScalarTensorPhase:
    def __init__(self,M1,M2,q1,q2,p1,p2,ms,c3):
        ...
        self.hbar_eVs = const.physical_constants["reduced Planck constant in eV s"][0] # UNITS: eV s
        ...
        self.msBar = (self.G * self.M * self.ms) / (self.c**3 * self.hbar_eVs)
    ...
    def GR_0PN(self, f, tc, phic):
        v = (np.pi * self.M * self.G * f / self.c**3)**(1/3)

        return 2 * np.pi * f * tc - phic - np.pi/4 + 3 / (128 * self.nu * v**5)
    
    def Psi_E(self, f):
        '''
        :param f: Gravitational wave frequency in Hz (twice the orbital frequency)
        :return: 0PN correction due to modified Kepler relation
        '''
        v = (np.pi * self.M * self.G * f / self.c**3)**(1/3)
        print("x =", v**2)
        
        first_term = 5 * self.QBar / (6 * self.nu * self.msBar ** (5 / 2)) * (upper_gammainc(5 / 2, self.msBar / v ** 2) + upper_gammainc(7 / 2, self.msBar / v ** 2) + 2 * upper_gammainc(9 / 2, self.msBar / v ** 2))
        second_term = - v ** 3 * 5 * self.QBar / (6 * self.nu * self.msBar ** 4) * (upper_gammainc(4, self.msBar / v ** 2) + upper_gammainc(5, self.msBar / v ** 2) + 2 * upper_gammainc(6, self.msBar / v ** 2))
    ...
    def Psi_Minus1PN(self, f):
        ...
        return float(np.real(np.heaviside((v**3/self.msBar) - 1, 1) * (self.delta_q**2 * 5 * 1/(896 * self.nu**3) * (first_term + second_term + third_term))))
    ...
    def Psi_0PN_p(self, f):
        ...
        second_term = - (6 * 2**(2/3) * 3**(1/2) * v**3 * sc.gamma(1/3)**3) / (187 * np.pi * self.msBar**(8/3))
    ...
    def Psi_0PN_l2(self, f):
        ...
        return float(np.real(np.heaviside((v**3/self.msBar) - 1/2, 1) * self.Xi_q**2 /(32*self.nu**3) * (first_term + second_term + third_term)))
